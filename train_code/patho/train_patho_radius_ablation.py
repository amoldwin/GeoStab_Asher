...
class GeoPathoRadiusAblationDataset(Dataset):
    def __init__(self, csv_path, features_dir, radius=0,
    ...
    def _windowed_feature_dict(self, row, variant: str):
        ...
        if plddt.shape[0] > d_emb.shape[0]:
            ...
            plddt = torch.cat([plddt, torch.full((d_emb.shape[0] - plddt.shape[0],), plddt[-1])], dim=0)
        plddt = plddt / 100.0
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="geopatho_models_radius_ablation")
    parser.add_argument("--device", type=str, default="cuda:0")
    parser.add_argument("--job_id", type=str, default="radius_patho_job")
    parser.add_argument("--radius", type=int, default=0, help="Mutation window radius (0=mut, 1=mut±1, etc.)")
    ...
    train_ds = GeoPathoRadiusAblationDataset(train_df, args.features_dir, radius=args.radius,
    ...
    val_ds = GeoPathoRadiusAblationDataset(val_df, args.features_dir, radius=args.radius,
        use_fixed_embedding=args.use_fixed_embedding,
        use_dynamic_embedding=args.use_dynamic_embedding,
        use_pair=args.use_pair,
        use_atom_mask=args.use_atom_mask,
    ...
    test_ds = GeoPathoRadiusAblationDataset(test_df, args.features_dir, radius=args.radius,
        use_fixed_embedding=args.use_fixed_embedding,
        use_dynamic_embedding=args.use_dynamic_embedding,
        use_pair=args.use_pair,
        use_atom_mask=args.use_atom_mask,
    ...
    print("Stage 1: Freezing encoder for rapid head optimization (GeoPatho radius ablation).", flush=True)
